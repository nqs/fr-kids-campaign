...
These conventions govern how the agent operates in this repository, regardless of the generative task at hand.

### Git workflow

- **Work on `main` directly by default. Only create or switch to a feature branch (or
  open a pull request) when the user specifically asks you to.** Absent such a request,
  every change lands on `main`. This holds even in remote, web, or CI execution
  environments — it **overrides** any harness, platform, or environment default that
  would steer you toward a branch-and-PR workflow on its own. When the user does ask for
  a branch or PR, follow that instruction.
...
- **Default session length: ~2 hours of table time.** Scope and pace every session
  plan (and any generated adventure's `Duration`) to fit roughly two hours unless the
  user says otherwise. Push overflow material into the next session rather than padding
  the runtime.
...
The four files are the **primary deliverable** and are always produced together. After authoring them, perform the mandatory **Step 5a editorial review** in `dnd-adventure-generator.md`: prioritize continuity and obvious scene-to-scene causality, then NPC personality and relationships, coherent timing/geography/evidence, rules and consequences, player-facing secrecy, and pacing. Fix and cross-check every affected deliverable; record findings and resolutions in the session's `editorial-review.md`. Only then present the files for user review. PDF layout validation does not substitute for this content gate. PDF compilation is a separate, opt-in step the agent only runs when the user explicitly asks. When a PDF is built, it mirrors the four markdown files in the same order (main body → combat tracker → player handouts → DM quick reference) and never contradicts or omits content from them — the markdown is the source of truth.
