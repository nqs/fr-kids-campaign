...
- **If the build fails with "N image(s) are missing local files":** present the full list of missing image descriptions to the user and ask whether they want to regenerate them via the Gemini MCP tool. If yes, regenerate each one, save the jpg locally, update `local_path` in `images.json`, and then re-run the build. Never silently skip a missing image — the PDF must contain all images or the build must fail.
