...
- Required node mappings:
  ...
  - `image` (`![alt](url)`) → stream the URL into a `BytesIO` with `urllib` or `requests` and emit an `Image()` sized to fill an 8.5"×11" page (max 7.2"×9.8") while preserving the captured aspect ratio (see `images.json` below). No PIL/Pillow. **Every image renders full-page** — portraits, monster art, location scenes, and tactical maps alike. There is no inline / thumbnail variant. The renderer pairs a heading with the image when the only content on the current page would otherwise be that heading (and an optional short intro paragraph): heading sits at the top, image fills the rest at max 7.2"×9.0". This avoids orphaned heading pages while still printing the image at full-page size.
...
- **Image sizing.** Every image renders **full-page on an 8.5"×11" sheet** — sized to fill the printable area while preserving aspect ratio. This applies uniformly to portraits, monster art, location scenes, and tactical maps. Step 4 captures `{description, url, aspect_ratio}` for each image; persist that list as `images/images.json` in the session folder so the renderer can letterbox correctly without re-querying Gemini. Lookup is by URL; if a URL isn't found, fall back to a 4:3 default and warn. Generate every image at high enough resolution to print at full 8.5"×11" size — portraits and scene art at the same resolution tier as tactical maps.
- **Page-break placement.** The renderer adds page breaks so each image lands on a fresh page and so headings don't strand alone on near-empty pages. Concretely: when an `![alt](url)` is the only content emitted since the last page break (or follows just a heading, or a heading plus one short intro paragraph), the renderer keeps the heading on the same page as the image and sizes the image at max 7.2"×9.0" so the pair fits together. Otherwise the image gets a full page on its own (max 7.2"×9.8"). Adjacent page breaks are coalesced — there are no blank pages.
...
- **Portraits**: `aspect_ratio="3:4"`, painterly fantasy style, neutral background. Must match the text description exactly — armor, species, distinguishing features, attitude. Generated at full-page resolution — every portrait prints on its own 8.5"×11" page in the handout appendix.
- **Location art**: `aspect_ratio="16:9"` for scene/landscape illustrations. Generated at full-page resolution — every location scene prints on its own 8.5"×11" page (letterboxed top/bottom to preserve aspect).
...
- Title + type/alignment line + CR (XP). (No portrait on the card itself — every portrait renders on its own full page in the handout appendix, and stat-block cards stay compact for table use.)
...
- A **tactical map** image immediately under the heading, on its own line: `![Tactical Map — <encounter name>](<url>)`. The alt text starts with `Tactical Map` so the encounter section is self-labeling; the renderer prints it full-page like every other image (page break before and after, sized to fill the printable area).
...
- A level-3 heading naming a creature, followed by a key/value block (`AC`, `HP`, `Speed`, ability scores, traits, actions), becomes a **stat-block card** with parchment background and a thin accent border. Stat-block cards never embed images — portraits print on their own full pages in the handout appendix.
...
- **Every image — tactical maps, portraits, monster art, location scenes — prints full-page on an 8.5"×11" sheet.** Images never share a page with prose, tables, or other images. The single exception is a section heading (and an optional short intro paragraph) that would otherwise sit alone on a near-empty page — the renderer pairs that heading with the image on the same page, with the image sized at max 7.2"×9.0" to leave room for the heading at the top. The renderer inserts page breaks before each image and coalesces adjacent page breaks so there are no blank pages.
