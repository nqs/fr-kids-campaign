---
tags: [campaign/session-4, dm-quick-ref, dnd-5e]
session: "004"
adventure: The Greengrass Greengage Affair
section: dm-quick-ref
---

# DM Quick Reference — The Greengrass Greengage Affair
*Session 004 · Print and keep at the table · Full detail in file 1 (adventure) and file 2 (combat)*

---

## Scene Order

| # | Scene | Key mechanic | DM flag |
|---|-------|--------------|---------|
| 1 | Anchor Decision (in the Shrine) | Choice: Smash / Steal / Feed / Withdraw | Yinu Arcana DC 17 for Feed (expertise) |
| 2 | Walk Back | Loot divvy · Brynn's letter · scene setting | Greengrass-eve ridge moment |
| 3 | Ashaba House Debrief | Pay 600 gp + bonus · **Level 6** | Print L6 sheets; warmer tone than S2 |
| 4 | Opening Procession | Read-aloud, station announcements | Five stations + Nema placeholder |
| 5 | Five Stations (montage) | 3-check skill challenges, DC 13/15/17 | Cut between PCs; ~5 min/turn max |
| 6 | The Bonfire | Ribbons + Lady Ulphor's toast | False moment of safety |
| 7 | **The Crash** | Antlered Herald + 2× Sprite Outriders | See combat tracker |
| 8 | Aftermath | Sela opens up · Brynn/Orvyn beats · Quillip hook | End session on the bonfire lit |

---

## Anchor Choice — Pay Table

| Choice | Anchor bonus | Side effects |
|---|---|---|
| **Smash** | +100 gp | Loud message; drow know |
| **Steal** | +200 gp | Orvyn gets the disc |
| **Feed** *(Harper)* | +300 gp | Yinu Harper marker; future listening payoff |
| **Withdraw** | +0 | Retaliation clock accelerates |

**Baseline pay:** 600 gp split (120 gp / PC) regardless of anchor choice.

---

## Station Skill Challenge — Universal Ladder

| Successes | Result |
|---|---|
| **3 of 3 — Gold** | Prize + Heroic Inspiration + personal NPC scene |
| **2 of 3 — Silver** | Prize |
| **1 of 3 — Bronze** | NPC scene |
| **0 of 3** | A line, no penalty |

**Cash prize per ribbon:** Gold 25 gp · Silver 10 gp · Bronze 0 gp (out of Lady Ulphor's purse).

---

## Stations — At-a-Glance

| PC | Station | Host | DCs | Special |
|---|---|---|---|---|
| **Kto** | Riverbank Archery | Helm + Rellan | AC 13 / WIS 15 / AC 17 | Gold = **public Helm-rift closure** |
| **Fiorn** | Bardic Showcase | Halbren Vass *(new)* | PERF 13 / DECEP 15 / PERF 17 | Use **Encouraging Song** for closer |
| **Yinu** | Arcane Stall | Orvyn Tal | ARC 13 / INV 15 / ARC 17 | Plants: signal-stone + numbing draught |
| **Loric T.** | Dragonchess Open | Brynn | DC 13 / 15 / 17 (per game) | Brynn drops **Mistledale Council intel** |
| **Nalith** | The Greening | Sela + Quillip *(invisible)* | HERB 13 / PERF 15 / AH 17 | **Hawthorn berry plant**; drop regardless of ribbon |
| **Nema** | Footrace *(placeholder)* | TBD | DEX 13 / CON 15 / DEX 17 | **Replace once Lilli files Nema's sheet** |

---

## The Crash — One-Page Combat

### Setup
**When:** Lady Ulphor raises the firebrand at dusk.
**Trigger:** A weapon comes out before Lorindar finishes his sentence — Sela's ribbon recognized OR a PC moves first.
**Round budget:** 3 rounds max. Herald escapes when bloodied OR end of Round 3.

### Antagonists

**Lorindar Greenmantle, the Antlered Herald (CR 5)**
- AC **16** · HP **85** (bloodied **42**) · Speed 30 (40 mounted on stag)
- **Glaive:** +7, **1d10+4 slash + 1d8 force** (~14)
- **Bow:** +7, **1d8+4 pierce + 1d6 radiant** (~12), 150/600
- **Fey Step:** 30 ft., bonus, recharge 4–6 (at-will on planned exit)
- **Reproof of the Court:** disadvantage on 1 melee hit/round (reaction)
- **Standard-Bearer's Word:** free, 1/encounter — speak 1 sentence, all hear it within 60 ft.

**Sprite Outrider × 2 (CR 1 each)**
- AC **15** · HP **22** · Speed 80 fly (owl)
- **Shortbow:** +6, **1d6+4 pierce**, 40/160
- **Sleep-Tipped Arrow** (1/encounter): CON **DC 12** or incapacitated 1 round
- **Heart-Sight Arrow** (1/encounter): reads target intent; if "fear" not "malice," sprite holds fire

### Round-by-Round Cheat Sheet

| Round | Lorindar | Sprites |
|---|---|---|
| **1** | Speak invitation (free). Mount-charge to ~30 ft. of the first-aggressor PC. 2× glaive. | Heart-Sight on the 2 most aggressive PCs. Hold if "fear." |
| **2** | Continue glaive on same target. Reproof the second hit. | Sleep-Tipped Arrow on the loudest caster. |
| **3** | **Scripted Exit.** Fey Step + stag bolt 60 ft. into Living Wood. | Cover fire, then peel off. |

### Ending The Fight

| Outcome | When | Result |
|---|---|---|
| Round 3 timer | Always available | Herald escapes; standard delivered |
| Bloodied | HP ≤ 42 | Herald escapes early; same delivery |
| Sela intervenes | PC about to drop OR fight too lethal | Sela steps in; PCs hold; Herald bows out |
| Audience accepted | PCs hold weapons through speech | Best outcome; mark for S5+ |
| Lorindar killed | PC kill before Round 3 | **Bad.** Cost the table; war begins |

### Civilian Screen
**Helm + 4 reservists + Rellan + Sela** hold the line. **PCs do NOT have to triage civilians.** They are behind the militia.

---

## Audience Invitation (Standard-Bearer's Word — say it aloud!)

> *"My lord of the Bright Court Beneath extends to the five who walked his borders an audience at the next full moon, on neutral ground, the hawthorn-cleft of his choosing. The Lady of the Ashaba is welcome. You are welcome. The fire is welcome. Come."*

This is the line Lorindar MUST deliver before he leaves. **Drop it in Round 1 as a free action.** Once said, it is heard by all PCs and Lady Ulphor regardless of noise.

---

## Plants for Later (don't pay off this session)

| Plant | Source | Pays off in |
|---|---|---|
| Lolthite signal-stone fragment | Yinu Station C | Drow war (S5–6) |
| Vhaeraunian numbing draught vial | Yinu Station C | Drow war (S5–6) |
| Dried hawthorn berry | Nalith Station E (Quillip) | Fey audience (S5–6) |
| Anchor's fey-inscribed inner ring | Scene 1, Nalith's Druidic-sense | Future Sela conversation |
| Harper primer book | Yinu (Gold C or Feed) | Yinu's Harper recruitment arc |
| Mistledale Council vote (Brynn intel) | Loric Station D | Council arc (S5+) |
| Halbren's "where are you touring next?" | Fiorn Gold B | Fiorn recurring rival |

---

## Loose Ends This Session **Closes**

- **Helm rift** — publicly closed at Kto's Gold station OR at Helm's solo "I was wrong" beat in aftermath.
- **Brynn's letter** — Brynn knows Kto knows. Kto chooses when. Loric on-ramped via Dragonchess intel.
- **Unnamed fey lord** — named (sort of): **the Bright Court Beneath.** Lord's name still withheld for future reveal.
- **Sela's "consultations"** — Sela tells the party, plainly, in the aftermath.
- **Quillip's S3 silence about Sela's lord** — explained on stage.

## Loose Ends This Session **Opens**

- **Audience at the next full moon, hawthorn-cleft.** Mark for S5 or S6.
- **Yinu + Harpers.** Orvyn has tipped his hand.
- **Sela's arc** — she's now actively asking for help. Update factions.md §8.
- **Halbren Vass** — recurring rival for Fiorn.
- **The signal-stone / numbing draught** — Vhaeraunian retaliation clock is *audibly* ticking.

---

## NPCs Onstage (quick refs)

| NPC | Role today | Read-aloud anchor |
|---|---|---|
| **Lady Addee Ulphor** | Hosts festival, lights bonfire, "you will not be charged" | Warm, almost maternal |
| **Brynn Ashford** | Runs the Dragonchess Open; "Thursday afternoons" to Kto | Sharper; he's a Harper-adjacent operator |
| **Orvyn Tal** | Hosts Yinu's stall; **Harper recruitment** | Quiet, generous, watchful |
| **Captain Corwick Helm** | Judges Kto's archery; **Helm-rift closure** | "I was glad I was wrong about him." |
| **Rellan Tessar** | Shoots with Kto; friend-ladder beat | Quietly cheerful, on his feet again |
| **Sela Wyndmere** | Greening with Nalith; **Crash trigger**; aftermath confession | Knew the standard. Will not lift a weapon. |
| **Halbren Vass** *(new)* | Fiorn's heat; recurring rival seed | Smug, charming, professionally jealous |
| **Quillip Burr-and-Whistle** | Invisible at Nalith's station; drops hawthorn berry; aftermath wink | Hint of warning; chained Plot B hook |
| **Lorindar Greenmantle** *(new)* | The Antlered Herald; CR 5 fey | Courteous to a fault. Will not raise his voice. |

---

## Level-Up Reminders (Level 6 takeaways)

| PC | Class | Level 6 marquee |
|---|---|---|
| **Kto** | Fighter (Champion) | ASI/Feat (suggest **Resilient: WIS** or **Sharpshooter**) |
| **Fiorn** | Rogue (Thief) | Subclass feature usage (Fast Hands is already humming) — pick up an Expertise option if available |
| **Yinu** | Wizard (Order of Scribes) | 3rd-level spell slot expansion + scribe feature |
| **Loric T.** | Sorcerer (Draconic) | Sorcerous Restoration improvement, Metamagic option |
| **Nalith** | Druid (Circle of Dreams) | **Hidden Paths** (Circle of Dreams 6th-level feature — short-rest teleport, very fey-themed; thematic for tonight!) |
| **Nema** | TBD | TBD |

> [!flag] Print L6 reference sheets and bring them to the table.

---

## Timing Budget

| Block | Target | Slip-tolerance |
|---|---|---|
| Cleaning Up (Scenes 1–3) | 45–60 min | Move on at 75 if needed; trim debrief |
| Procession (Scene 4) | 10 min | — |
| Five Stations (Scene 5) | 75–90 min | Cap at 100; cut if stations drag |
| Bonfire (Scene 6) | 10 min | — |
| The Crash (Scene 7) | 30–40 min | **Hard stop at 45.** End on Sela if it's running long. |
| Aftermath (Scene 8) | 15 min | — |
| **Total** | **3:00–3:45** | Have a 15-minute break between Scene 5 and Scene 6 |

---

## DM Don'ts

- **Don't** kill Lorindar. (See file 2 — let it cost if it happens.)
- **Don't** put a civilian in danger. Helm screens. The kids do not have to triage.
- **Don't** force the letter or the Harpers. If kids walk past them, that's fine; both threads survive to next session.
- **Don't** lock Nema's station until her sheet is in.
- **Don't** explain the signal-stone or numbing draught. **Plant** them.
- **Don't** narrate Quillip dropping the berry — *let Nalith notice it.*
