...
2. **`<slug>-2-combat-tracker.md`** — DM combat tracker: per-encounter **tactical maps (full-page, printable, one map per page)**, tracker sheets, and stat-block cards for every non-PC combatant. The full specification lives in `dnd-adventure-generator.md` under the **Combat Tracker** section.
